...
def elt_dim_user():
    ...
    run_dbt = BashOperator(
        task_id='run_dbt',
        bash_command=f'dbt run --project-dir {project_dir} --profiles-dir {profiles_dir} --select dim_user',
    )

    wait_for_load_bike_usage_data >> run_dbt

dag_instance = elt_dim_user()